...
def processar_nf(conn, numero):
    ...
    cursor.execute(
        "SELECT FIRST 1 CDNOTASAIDA, DTSAIDAENTRADA FROM NOTASAIDA WHERE NUMNOTA = ? ORDER BY DTSAIDAENTRADA DESC",
        (numero,))
    row = cursor.fetchone()
    if not row:
        return None
...
def main():
    ...
    output_file = "relatorio_produtos_servicos.csv"
    ...
    with open(input_file, 'r', encoding='utf-8') as f_in, \
        ...
        for line in f_in:
            ...
            if info:
                ...
                writer.writerow([f"{tipo} {numero}", data_str, "", "", "", "", ""])
                ...
                for item in itens:
                    ...
                    writer.writerow([
                        ...
                        item[1],  # Descrição
                        formatar_qtd(item[2]),
                        formatar_valor(item[3]),
                        formatar_valor(item[4])
                    ])

                # Adiciona um espaço entre documentos
                writer.writerow([])
                
                print(f"OK: {tipo} {numero}")
            else:
                print(f"NÃO ENCONTRADO: {tipo} {numero}")
                writer.writerow([f"{tipo} {numero} (NÃO ENCONTRADO)", "", "", "", "", "", ""])
